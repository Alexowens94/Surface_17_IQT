import time
from compiled_surface_code_dressed_state_error_model import *
import numpy as np
...
run_name = 'test_log_e_rate_til_fail_pleak0'
...
correction_table = load_lookup_table("C://Users//ds572//OneDrive - University of Sussex//Sussex PhD//Surface_17_IQT//correction_table_depolarising.json")
...
for _ in range(number_of_runs):
    ...
    while True:
        ...
        quiescent = np.array(stabilizer_cycle(data, ancilla, leaked_q_reg, eng, reset=True,
                                              p_leak=0, pxctrl=0, pyctrl=0, pxxctrl=0, pmot=0))

        ## Error correction cycle
        prev_syndrome=np.array(quiescent)

        syndrome = np.array(stabilizer_cycle(data, ancilla, leaked_q_reg, eng, reset=True,
                                             p_leak=p_leak, pxctrl=pxctrl, pyctrl=pyctrl, pxxctrl=pxxctrl, pmot=pmot))

        print("measured first syndrome")
        ...
        if np.all((flips_a == 0)):
            ...
            syndrome = np.array(stabilizer_cycle(data, ancilla, leaked_q_reg, eng, reset=True,
                                                 pyctrl=pyctrl, pxctrl=pxctrl, pxxctrl=pxxctrl, pmot=pmot, p_leak=p_leak))

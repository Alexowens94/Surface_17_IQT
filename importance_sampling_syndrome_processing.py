...
runs = 1000
model = 'PDD_model'
filename_s1_impsamp = "ImpSamp_1000_syndrome_process_s1subsets_{}_nbarheat".format(model)
filename_s2_impsamp = "ImpSamp_1000_syndrome_process_s2subsets_{}_nbarheat".format(model)
...
cz_comp = False
types = ['Rx_ctrl]', 'Ry_ctrl', 'Rxx_ctrl', 'dephase', 'heat']
locations = [12, 18, 24, 78, 24]  # locations errors can occur in pdd model in round 1
locations_both_rounds = [2*12, 2*18, 2*24, 2*78, 2*24] # locations for errors to occur, 2 rounds
prob_list = [1e-4, 1e-4, 5e-3, 1e-3, 5e-3]
gates_per_timestep = 3
num_steps = 8
pxxctrl_heating_s1 = []
pxxctrl_heating_s2 = []
for i in range(num_steps):
    pxxctrl_heating_s1 += gates_per_timestep*[(i+1)*prob_list[3]]
    pxxctrl_heating_s2 += gates_per_timestep*[(i+1+num_steps)*prob_list[3]]
...
#locations[2]*[prob_list[2]],
prob_lists_s2 = [[prob_list[0]], [prob_list[1]], pxxctrl_heating_s2, [prob_list[3]], [prob_list[4]]]
prob_lists_both_rounds = [[prob_list[0]], [prob_list[1]], pxxctrl_heating_s1+pxxctrl_heating_s2, [prob_list[3]],
                          [prob_list[4]]]
print('ps1s2 {}'.format(prob_lists_both_rounds))
...
results_dict = calculate_significant_subsets_incrementally(locations, prob_lists_s1, cutoff_weight=1e-6)
...
for i, eset in enumerate(s1_significant_subsets):
    ...
    eset_log_e_rate, eset_s2_rate, fails, s2count = s1_calculate_log_e_rate_error_subset(runs, correction_table, model, eset, locations,
                                                        bitflip_table, prob_lists_s1, cz_comp)
    key = str(eset)
...
s2res = s2sim_calculate_significant_subsets_incrementally(locations, prob_lists_s1, prob_lists_s2, s2_rate_dict,
                                                          s1_significant_subsets, significant_subsets_s1_and_s2)
